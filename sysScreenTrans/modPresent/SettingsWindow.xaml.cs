...
/// <summary>
/// 系統匣「設定…」視窗：設定 API 金鑰（寫使用者環境變數、不落地）與語音來源／模型／嗓音、
/// 查詢模型（寫 appsettings.json）。「測試發音」以當前設定即時試聽。
/// </summary>
public partial class SettingsWindow : Window
{
    private static readonly string[] Voices = { "nova", "shimmer", "coral", "alloy", "echo", "fable", "onyx", "sage" };
    ...
    public SettingsWindow(AppConfig current)
    {
        ...
        SelectByTag(ProviderBox, current.TtsProvider);
        SelectByTag(ModelBox, current.TtsModel);
        SelectByTag(VoiceBox, string.IsNullOrWhiteSpace(current.Voice) ? "nova" : current.Voice);
    ...
    private void OnTest(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            _testSvc = cfg.TtsProvider.Equals("windows", StringComparison.OrdinalIgnoreCase)
                ? new SpeechService(cfg.Voice)
                : new OpenAiSpeechService(key, cfg.TtsModel, cfg.Voice, cfg.TimeoutSec, fallback: new SpeechService(null));
