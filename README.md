...
| 開啟主視窗 | 點工作列 LingoIsland 按鈕／`Alt+Tab`，或系統匣圖示 → 開啟**統一主視窗**（上方分頁：**Dictionary**／螢幕擷取／筆記／歷史／選項／關於） |
...
| 移除 | 結束程式後於「設定 → 應用程式」解除安裝（Portable 版刪除資料夾）＋刪除環境變數；個人資料（筆記/歷史/情境/設定，`%APPDATA%\LingoIsland`）視需要自行刪除 |
...
2. 系統自動抓該影片的英文字幕，逐句列在左側；右側嵌入 YouTube 播放器。
...
> **字幕來源**：優先抓影片的**人工（真人）字幕**（乾淨句級、學習體驗最佳）；若該片只有 YouTube **自動字幕**（機器轉錄、逐字滾動較破碎），會退而取用並在狀態列**明確提示**——建議改選有正式字幕的影片。
> 需要：Windows 11（內建 WebView2；缺失時 Video 分頁明確提示安裝、不影響其他功能）、本機 `yt-dlp`；查詢一樣用你自備的 OpenAI 金鑰。**本工具只取字幕文字、不下載影片內容。**
> 本版只做「學習查詢」（點字查、存筆記）；限定角色的「代言／跟讀練習」與自動字幕之語音重轉錄（Whisper）、完整影片庫管理為後續版本。
