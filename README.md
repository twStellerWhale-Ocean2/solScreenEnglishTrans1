...
| 截圖管理 | 「螢幕截圖」分頁（v2.9.0 版面）：**左＝截圖清單**（Clear all 置頂、依 theme 篩選）｜**右上＝擷取控制**（Capture Screen＋快捷鍵 Change）｜**右下＝放大預覽**（＋Delete）。每次擷取自動保存（縮圖含擷取時間＋使用中主題） |
...
2. 版面（v2.9.0 起）：**左＝影片清單到頂**（上方 Clear all；載過的影片留在此、可點切換載入、依 theme 篩選、可刪除）｜ **右上＝網址輸入**（Load）｜ **右下＝播放器＋當前句字幕帶＋控制與逐句英文字幕清單**。欄寬可拖拉調整。
...
### 螢幕截圖頁與影片頁版面重排（v2.9.0）

兩頁改成**左清單、右操作**的一致版面：

- **螢幕截圖頁**：左＝截圖清單（**Clear all** 置頂、依 theme 篩選）｜右上＝水平擷取控制（Capture Screen＋快捷鍵 Change）｜右下＝放大預覽（＋Delete）。
- **影片頁**：左＝影片清單到頂（**Clear all** 置頂、依 theme 篩選、Delete）｜右上＝網址輸入（Load）｜右下＝播放器＋字幕（現有影片功能）。影片清單新增 **Clear all**（一次清空清單、含確認）。

![螢幕截圖頁：左清單（Clear all／theme 篩選）＋右上水平擷取控制＋右下預覽](docs/manual-assets/capture-layout.png)

![影片頁：左影片清單到頂（Clear all）＋右上網址＋右下播放器/字幕](docs/manual-assets/video-layout.png)
