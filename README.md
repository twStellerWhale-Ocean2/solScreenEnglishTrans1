...
- 可設定**應用情境提示**（如「中世紀奇幻 RPG，用遊戲用語翻譯」），讓 AI 依情境翻得更貼切（選填、可隨時修改或清空）
...
| 開啟查詢歷史 | 結果視窗「展示歷史紀錄」按鈕，或常駐主控入口開啟歷史視窗 |
| 回顧歷史 | 左側選日期 → 右側該日紀錄（新的在上）；每筆可 `▶ 播音`、`檢視`（回中英詳情）、`刪除` |
| 清除歷史 | 歷史視窗頂部「清除全部」；或逐筆「刪除」 |
| 加入我的筆記 | 結果視窗「加入我的筆記」，或歷史條目「＋筆記」→ 右下角閃示「已加入」（已收藏過則提示「已在筆記中」） |
| 開啟我的筆記 | 結果視窗／常駐主控入口／系統匣「我的筆記」→ 左資料夾、右條目 |
| 分類與排序 | 左側新增／更名／刪除資料夾、把條目歸類；右側拖曳條目前端 `≡` 上下調整順序（重啟沿用） |
| 筆記操作 | 每筆可 `▶ 播音`、`檢視`（回中英詳情）、`刪除` |
| 設定情境提示 | 系統匣／主控頁「設定」→「應用情境提示」填入目前應用主題（如遊戲類型），提升翻譯貼切度；留空＝維持預設翻譯 |
...
| `paramContextHint` | （空） | 應用情境提示；非空時作為「參考、非指令」併入查詢提示以輔助翻譯；建議由「設定 → 應用情境提示」填寫；留空＝維持預設翻譯行為 |
