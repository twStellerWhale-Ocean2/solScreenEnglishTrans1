...
- 對筆記**練習發音**：在筆記卡片播音鈕旁**按住燈泡鈕錄音、放開由 AI 評分**，唸得夠標準燈泡就亮（練習成績本機保存、可一鍵清空重練）
...
| 我的筆記 | 主視窗「筆記」分頁：左側**多層資料夾樹如檔案總管**——頂部「建立資料夾」、節點**右鍵選單**（新增子資料夾／更名〔`F2` 原地編輯〕／刪除〔`Del`〕）、**同層自動依名稱排序**、拖曳只改所屬資料夾（目標夾高亮）；右側該夾條目可拖曳排序（顯示插入位置線）**或拖到左側資料夾改歸屬**，右上「**Clear Practice（清空練習紀錄）**」把該夾所有燈泡歸零；條目**右鍵**＝`▶ 播音`／`檢視`／`底色`／`刪除`，**雙擊＝檢視**，**行尾播音鈕旁有發音練習燈泡鈕** |
| 發音練習 | 筆記卡片播音鈕旁的**燈泡鈕**（預設不亮）：**按住錄音、放開送 AI 評分**（錄音會上傳 OpenAI 評分、不留存本機）；**放開後顯示分數**（如「發音 88／80」），達及格門檻→燈泡點亮、成績存於本機（重啟仍在）；唸太短／沒麥克風／沒網路會**各自明確提示**、燈泡不誤亮；右上「Clear Practice」把整個資料夾的燈泡歸零重練 |
...
| `paramPronPassThreshold` | `80` | 發音練習及格門檻（0–100）；唸出的分數達到此值，該筆筆記燈泡才點亮。建議由「選項」分頁調整 |
| `paramPronModel` | `gpt-4o-mini-audio-preview` | 發音評分使用的 OpenAI 音訊模型（須支援語音輸入）；沿用同一把 `OPENAI_API_KEY`，不另設金鑰 |
...
- 對某則筆記按住燈泡鈕唸一次、放開後 AI 回評分：達門檻燈泡亮、重啟後仍亮；「Clear Practice」後該夾燈泡全滅；無麥克風或錄音太短時明確提示、燈泡不會誤亮。
...
- **發音練習燈泡按了沒反應／唸完不亮？** 確認 Windows 隱私權→麥克風已允許桌面應用存取（設定 → 隱私權與安全性 → 麥克風），錄音時說得夠久（太短會忽略），且能連上 OpenAI；燈泡是否點亮取決於分數是否達「選項」分頁設定的及格門檻。
