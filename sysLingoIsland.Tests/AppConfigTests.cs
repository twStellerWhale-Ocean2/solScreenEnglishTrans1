...
/// <summary>
/// [etyCfg自訂sysLingoIsland組態] 往返與容錯（Issue #9：移除 paramTtsProvider／paramTtsModel）。
/// </summary>
public class AppConfigTests
{
    private static string TempPath() => Path.Combine(Path.GetTempPath(), $"lingoisland-cfg-{Guid.NewGuid():N}.json");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"lingoisland-cfgdir-{Guid.NewGuid():N}");
    ...
    {
        // Issue #51：首次於 %APPDATA%\LingoIsland 存檔時目錄可能不存在
        var dir = TempDir();
