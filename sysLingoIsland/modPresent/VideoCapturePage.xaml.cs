...
public partial class VideoCapturePage : System.Windows.Controls.UserControl
{
    ...
    private readonly VideoSubtitleStatusStore _statusStore = new();  // 字幕狀態快取（#188→增量6′）：獲得框存入字幕**來源指標**（網址或本機路徑，#193）＋載入來源，載入未快取時取用建立字幕
    private readonly List<AcquireEntry> _picked = new();             // 【獲得】已選之本機字幕檔（#193）：選檔/拖放後之預掃描結果（僅讀檔頭取影片 ID，未全檔解析）
        ...
        Func<ISpeechService?> speechProvider)
    {
        ...
        // 本機自製字幕檔（#193）：選檔鈕／全部清除／拖放至輸入框。拖放回饋**拖曳中才顯示**，非支援檔型顯拒收（None）。
        AcqPickBtn.Click += (_, _) => PickSubtitleFiles();
        AcqClearFilesBtn.Click += (_, _) => { _picked.Clear(); RefreshPickedFiles(); SetStatus("已清空字幕檔清單。"); };
        // 掛於整張【獲得】卡片（USR 回饋）：拖放範圍＝整個「加入影片」區塊，非僅輸入框。
        // 用 **Preview（穿隧）** 掛在父層：游標落在卡片內任一子元素（輸入框、按鈕、已選檔清單、空白處）皆先經此處理，子元素不必各自實作。
        SearchPane.PreviewDragOver += (_, e) =>
        {
            e.Effects = DroppedSubtitleFiles(e).Count > 0 ? System.Windows.DragDropEffects.Copy : System.Windows.DragDropEffects.None;
            e.Handled = true; // 必須攔下：TextBox 預設只接受文字放置，不處理 FileDrop
        };
        SearchPane.PreviewDrop += (_, e) =>
        {
            var files = DroppedSubtitleFiles(e);
            if (files.Count == 0) { return; }   // 非檔案放置（如純文字拖入輸入框）→ 交還子元素預設行為
            e.Handled = true;
    ...
    private async Task LoadVideoAsync(string id, bool addToStore, string? listItemId = null)
    {
        ...
        string? freshRaw = null;        // #218：首次載入之字幕來源原文（與 cues 同點落地 subtitle-source）
        if (cached is null)
        {
            ...
            var fetched = await FetchParseSubtitleAsync(subtitleUrl); // 內部設狀態；null＝取檔/解析失敗或無時間碼→中止
            if (fetched is null)
            {
            ...
            if (!ConfirmLoadSummary(fresh, id, srcLabel)) // 顯示綁定影片＋摘要（句數/說話人/時長）供一眼確認對得上；取消＝不載入、不拆除目前畫面
            {
                if (listItemId is not null) { RefreshVideoList(); }
                return;
            }
        ...
        try
        {
            ...
            else // 首次：已於上方取檔＋解析＋摘要確認（fresh 必非空且含時間碼）→直接存檔載入
            {
                ...
                _subs.SaveSource(id, CurrentVideoTitle(), freshRaw); // #218 三件式：來源原文同點落地 subtitle-source.txt
            }
            SetCues(cues);
    ...
    /// </summary>
    private async Task<(IReadOnlyList<SubtitleCue> Cues, string Raw)?> FetchParseSubtitleAsync(string subtitleUrl)
    {
        string raw;
        try
        {
            // 取得端二選一（#193）：以「是否 http(s) 開頭」單點分流——網址走 curl（繞 Cloudflare）、否則視為本機路徑走唯讀讀檔。
            var isUrl = subtitleUrl.StartsWith("http://", StringComparison.OrdinalIgnoreCase)
                     || subtitleUrl.StartsWith("https://", StringComparison.OrdinalIgnoreCase);
            SetStatus(isUrl ? "讀取字幕檔中…" : $"讀取字幕檔 {TranscriptFile.SafeName(subtitleUrl)} 中…");
            raw = isUrl
                ? await TranscriptFetch.FetchAsync(subtitleUrl, CancellationToken.None)
                : await Task.Run(() => TranscriptFile.Read(subtitleUrl));
        }
        ...
        if (det.Any(c => c.StartSec.HasValue)) { return (det, raw); } // #218：raw 隨 cues 回傳，供落地 subtitle-source
        ...
        return (aiCues, raw); // #218：AI 抽取路徑亦回傳 raw（送 AI 分析之原文）
    }
    ...
    /// </summary>
    private bool ConfirmLoadSummary(IReadOnlyList<SubtitleCue> cues, string videoId, string? source)
    {
        ...
        // #193：**顯示將綁定之影片**——否則使用者無從察覺綁錯片（尤以影片網址取自字幕檔檔頭時）。
        var srcLine = string.IsNullOrWhiteSpace(source) ? "" : $"\n字幕來源：{source}";
        var msg =
            $"將綁定影片：{videoId}\nhttps://www.youtube.com/watch?v={videoId}{srcLine}\n\n" +
            $"字幕檔解析結果：\n{cues.Count} 句 · {timed} 句有時間 · 涵蓋 0:00–{FormatPos(lastSec)}\n{speakerLine}\n" +
            $"影片長度：約 {FormatPos(lastSec)}（依字幕最後一句起點推估；起播後以播放器實際長度為準）\n\n" +
    ...
    {
        // 已選本機字幕檔（#193）優先：單檔＝摘要確認後載入播放（沿用既有動線）；多檔＝彙總確認後批次加入、不自動播放。
        if (_picked.Count > 0) { _ = DoAcquireFromFilesAsync(); return; }
        ...
        // 記字幕來源指標到該片：LoadVideoAsync 未快取時即取此建立字幕（快取還原與載入來源一致）。
    ...
    /// <summary>
    /// 自已選之本機字幕檔建立（#193）。**單檔**＝沿用既有「摘要確認→載入播放」動線（僅來源改為本機路徑）；
    /// **多檔**＝批次加入而非批次載入播放。影片網址**檔內優先**，惟優先序不得靜默生效——檔內與輸入框同時有值且不同時，於狀態列明示採用與忽略項。
    /// </summary>
    private async Task DoAcquireFromFilesAsync()
    {
        var usable = _picked.Where(e => e.IsAddable || e.Status == AcquireStatus.AlreadyExists).ToList();
        if (usable.Count == 0)
        {
            SetStatus("已選的字幕檔都不能加入——請看清單上每個檔案的狀態說明。");
            return;
        }

        // 輸入框若也給了影片網址：單檔時可作為檔內無網址之退路，並於衝突時明示忽略項。
        var typedId = ExtractUrls(AcqInputBox.Text).Select(ExtractVideoId).FirstOrDefault(v => v is not null);

        if (usable.Count == 1 && _picked.Count == 1)
        {
            var one = usable[0];
            var id = one.VideoId ?? typedId;
            if (id is null)
            {
                SetStatus($"{one.FileName} 的檔頭沒有影片網址——請在上面的框補上該片的 YouTube 網址。");
                return;
            }
            if (one.VideoId is not null && typedId is not null && typedId != one.VideoId)
            {
                SetStatus($"已採用檔內的影片（{one.VideoId}）；輸入框的 {typedId} 未使用。");
            }
            if (_subs.TryLoad(id) is not null)
            {
                var ow = System.Windows.MessageBox.Show(System.Windows.Window.GetWindow(this),
                    "這支影片已有字幕紀錄。要覆寫、重新建立嗎？（取消則保留現有紀錄）", "影片已加入",
                    System.Windows.MessageBoxButton.OKCancel, System.Windows.MessageBoxImage.Question);
                if (ow != System.Windows.MessageBoxResult.OK) { SetStatus("已保留這支影片的現有字幕。"); return; }
                _subs.Remove(id);
            }
            _statusStore.SaveWeb(id, found: true, source: "本機字幕檔", transcriptUrl: one.Path);
            _picked.Clear();
            RefreshPickedFiles();
        ...
        await RunBatchAsync(usable);
    }

    /// <summary>
    /// 多檔批次加入（#193）：彙總確認 → 逐檔解析並存檔加入清單 → 結果摘要。**全程不觸發 AI**——
    /// 免費解析抽不到時間者標「需 AI 抽取·已略過」放著不動（批次絕不無聲燒額度）。可取消：已完成者保留、未完成者不建立。
    /// </summary>
    private async Task RunBatchAsync(IReadOnlyList<AcquireEntry> entries)
    {
        if (System.Windows.MessageBox.Show(System.Windows.Window.GetWindow(this),
                AcquireBatch.ConfirmText(entries), "批次加入影片",
                System.Windows.MessageBoxButton.OKCancel, System.Windows.MessageBoxImage.Question)
            != System.Windows.MessageBoxResult.OK)
        {
            SetStatus("已取消批次加入——已選的字幕檔仍保留在清單上。");
            return;
        }

        var added = new List<string>();
        var skipped = new List<AcquireEntry>();
        var failed = new List<(AcquireEntry Entry, string Reason)>();
        var themeActive = ThemeStore.GetActive(_themes.Load());

        AiActionWindow.RunAndShow(System.Windows.Window.GetWindow(this), "批次加入影片",
            async (report, ct) =>
            {
                for (var i = 0; i < entries.Count; i++)
                {
                    ct.ThrowIfCancellationRequested();      // 取消：已完成者保留、未完成者不建立
                    var e = entries[i];
                    report($"處理中 {i + 1}/{entries.Count}：{e.FileName}");
                    if (e.VideoId is null) { skipped.Add(e); continue; }
                    ...
                    try
                    {
                        ...
                        {
                            skipped.Add(e with { Status = AcquireStatus.Unreadable, Detail = "沒有時間軸——需 AI 抽取，已略過（可單檔處理）" });
                            continue;
                        }
                        _subs.Save(e.VideoId, e.FileName, isAutoGenerated: false, cues);
                        _subs.SaveSource(e.VideoId, e.FileName, rawText); // #218 三件式：批次路徑同落來源原文
                    ...
                    catch (Exception ex) { failed.Add((e, ex.Message)); }
                }
                report(AcquireBatch.ResultText(added.Count, skipped, failed));
                return null;    // 免費動作、無費用可報
            },
        ...
        RefreshPickedFiles();
        RefreshVideoList();
        SetStatus($"批次加入完成：成功 {added.Count} 部、略過 {skipped.Count} 個、失敗 {failed.Count} 個。已加入「內容」子頁籤，未自動播放。");
    }
    ...
    // ---- 本機自製字幕檔（#193）：選檔／拖放→預掃描（只讀檔頭取影片 ID）→單檔載入或多檔批次加入 ----

    /// <summary>自拖放事件取出**白名單內**之檔案路徑（純判斷、不讀檔）；非 FileDrop、資料夾或非白名單副檔名一律不計，供 DragOver 決定顯示可放置或拒收。</summary>
    private static IReadOnlyList<string> DroppedSubtitleFiles(System.Windows.DragEventArgs e)
    {
        if (!e.Data.GetDataPresent(System.Windows.DataFormats.FileDrop)) { return Array.Empty<string>(); }
        if (e.Data.GetData(System.Windows.DataFormats.FileDrop) is not string[] paths) { return Array.Empty<string>(); }
        return paths.Where(p => !Directory.Exists(p) && TranscriptFile.HasAllowedExtension(p)).ToList();
    }

    /// <summary>開檔案對話框選一或多個本機字幕檔（記憶上次目錄，桌面慣例）；篩選字串與白名單同源於 <see cref="TranscriptFile"/>。</summary>
    private void PickSubtitleFiles()
    {
        var dlg = new Microsoft.Win32.OpenFileDialog
        {
            Filter = TranscriptFile.DialogFilter,
            Multiselect = true,
            Title = "選擇字幕檔",
        };
    ...
    /// <summary>
    /// 併入選取／拖入之檔案並預掃描（#193）：先以純函式 <see cref="AcquireBatch.Merge"/> 去重與套檔數上限（逾限明訊拒收、不默默截斷），
    /// 再**非同步**逐檔只讀檔頭區取影片 ID（不全檔解析、不阻塞 UI）；單檔失敗只影響該列。
    /// </summary>
    private async Task AddPickedFilesAsync(IEnumerable<string> paths)
    {
        var merged = AcquireBatch.Merge(_picked.Select(p => p.Path), paths, out var rejected);
        var scanned = new List<AcquireEntry>(merged.Count);
        foreach (var p in merged)
        {
            var known = _picked.FirstOrDefault(e => string.Equals(e.Path, p, StringComparison.OrdinalIgnoreCase));
            scanned.Add(known ?? await PreScanAsync(p));   // 已掃過者沿用，免重複 IO
        }
        _picked.Clear();
        _picked.AddRange(AcquireBatch.MarkDuplicateVideoIds(scanned));
        RefreshPickedFiles();
        SetStatus(rejected > 0
            ? $"已選 {_picked.Count} 個字幕檔；超過單批上限 {TranscriptFile.MaxBatchFiles} 個，有 {rejected} 個未加入。"
            : $"已選 {_picked.Count} 個字幕檔。");
    }

    /// <summary>單檔預掃描：副檔名／可讀性／檔頭影片 ID／是否已有字幕。**只讀檔頭區**（<see cref="TranscriptFile.ReadHeader"/>），不解析全檔。</summary>
    private async Task<AcquireEntry> PreScanAsync(string path)
    {
        if (Directory.Exists(path) || !TranscriptFile.HasAllowedExtension(path))
        {
    ...
    {
        AcqFilesList.Items.Clear();
        // 依檔名自然排序（沿用 NotesStore.NaturalCompare 家規：數字段依數值，使 e02 排在 e10 前）
        foreach (var e in _picked.OrderBy(x => x.FileName, Comparer<string>.Create(NotesStore.NaturalCompare)))
        {
            AcqFilesList.Items.Add(PickedFileRow(e));
        }
    ...
    private System.Windows.FrameworkElement PickedFileRow(AcquireEntry e)
    {
        var grid = new System.Windows.Controls.Grid { Margin = new System.Windows.Thickness(2, 1, 2, 1) };
        ...
        var name = new System.Windows.Controls.TextBlock
        {
            ...
            VerticalAlignment = System.Windows.VerticalAlignment.Center,
            TextTrimming = System.Windows.TextTrimming.CharacterEllipsis,
            ToolTip = e.Path,   // 完整路徑只在 tooltip（絕對路徑含使用者名稱，防隨截圖外洩）
        };
        grid.Children.Add(name);

        var status = new System.Windows.Controls.TextBlock
        {
            Text = AcquireBatch.StatusText(e),
            FontSize = 10.5,
            Margin = new System.Windows.Thickness(10, 0, 8, 0),
            VerticalAlignment = System.Windows.VerticalAlignment.Center,
            Foreground = e.IsAddable
        ...
        var del = new System.Windows.Controls.Button
        {
            ...
            Height = 20,
            FontSize = 10,
            ToolTip = $"自清單移除 {e.FileName}",
            Focusable = true,
        };
        del.Click += (_, _) => { _picked.Remove(e); RefreshPickedFiles(); };
        del.KeyDown += (_, ev) => { if (ev.Key == Key.Delete) { _picked.Remove(e); RefreshPickedFiles(); ev.Handled = true; } };
        ...
        if (!e.IsAddable)
        {
            ...
            {
                Background = new System.Windows.Media.SolidColorBrush(System.Windows.Media.Color.FromArgb(0x55, 0xFF, 0xE9, 0xC8)),
...
window.li_dur=function(){return (ready&&player&&player.getDuration)?player.getDuration():-1;};
...
""";
    ...
    private void SetWatchUrl(string videoId)
    {
        ...
        WatchDurationText.Text = "";   // 新片先清空；下方以字幕推估、起播後再以播放器實際長度覆蓋
        WatchUrlRow.Visibility = System.Windows.Visibility.Visible;
    }

    /// <summary>
    /// 網址列尾端之影片長度（USR 回饋）：**先以字幕最後一句起點推估**（載入當下即可見、免等起播），
    /// 起播後由 <see cref="UpdateCurrentVideoTitleAsync"/> 以播放器 <c>getDuration</c> 之實際長度覆蓋。無已定時句則不顯示。
    /// </summary>
