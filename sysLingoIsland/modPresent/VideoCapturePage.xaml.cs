...
public partial class VideoCapturePage : System.Windows.Controls.UserControl
{
    ...
    private bool _isAuto;              // 目前字幕為自動生成（逐字滾動、較破碎）——供狀態提示；亦即 Row1 基底＝Auto（#189）
    private bool _manualAvailable;     // #189：本片人工字幕是否可用（Row1 切換用；載入即落 Auto＝人工不存在＝false）
    private bool _autoAvailable;       // #189：本片自動字幕是否可用（樂觀開啟；切換該軌失敗才標 false）
    private bool _switchingBase;       // #189：Row1 切換基底軌抓取中（防重入）
    ...
    private async Task LoadVideoAsync(string id, bool addToStore)
    {
        ...
        try
        {
            ...
            SetCues(cues);
            // #189：依載入到的軌別設定 Row1（Auto＝人工不存在→灰 Manual；Manual＝人工存在、Auto 樂觀可切）
            _manualAvailable = !_isAuto;
            _autoAvailable = true;
            UpdateBaseToggles();
    ...
    /// <summary>依目前基底（_isAuto）與各軌可用性更新 Row1 兩顆互斥切換鈕：按下當前軌、灰掉不可用或抓取中。</summary>
    private void UpdateBaseToggles()
    {
        var has = _cues.Count > 0;
        BaseAutoBtn.IsChecked = has && _isAuto;
        BaseManualBtn.IsChecked = has && !_isAuto;
        BaseAutoBtn.IsEnabled = has && _autoAvailable && !_switchingBase;
        BaseManualBtn.IsEnabled = has && _manualAvailable && !_switchingBase;
    }

    private void OnBaseAutoClick(object sender, System.Windows.RoutedEventArgs e) => _ = SwitchBaseAsync(wantAuto: true);
    private void OnBaseManualClick(object sender, System.Windows.RoutedEventArgs e) => _ = SwitchBaseAsync(wantAuto: false);

    /// <summary>
    /// 切換基底來源軌（#189）：重抓指定軌字幕、取代目前字幕並存檔。點目前軌＝不動作（維持按下態）。
    /// 該軌不存在→標記不可用、灰掉、還原。切換是重抓「乾淨基底」（Row1 為起點），會捨棄目前的說話人/YAML 疊加。
    /// </summary>
    private async Task SwitchBaseAsync(bool wantAuto)
    {
        if (_cues.Count == 0 || _currentVideoId is null || _switchingBase || _loading || _yamlEditing || _inferring || _transcribing || wantAuto == _isAuto)
        {
            UpdateBaseToggles(); // 還原按下態（防點當前軌把它 toggle 掉）
            return;
        }
        var id = _currentVideoId;
        var prevAuto = _isAuto;
        _switchingBase = true;
        BaseAutoBtn.IsEnabled = false; BaseManualBtn.IsEnabled = false; // 抓取中停用兩鈕；不動 IsChecked（避免與剛點下的按鈕狀態打架閃爍）
        SetStatus(wantAuto ? "Switching to auto captions…" : "Switching to manual subtitles…");
        try
        {
            var result = await _fetcher.FetchTrackAsync(id, wantAuto, _loadCts?.Token ?? CancellationToken.None);
            _isAuto = result.IsAutoGenerated;
            if (wantAuto) { _autoAvailable = true; } else { _manualAvailable = true; }
        ...
        catch (OperationCanceledException) { _isAuto = prevAuto; SetStatus("Switch canceled."); }
        catch (SubtitleException ex)
        {
            if (wantAuto) { _autoAvailable = false; } else { _manualAvailable = false; } // 該軌不存在→灰掉
            _isAuto = prevAuto;
            SetStatus(ex.Message);
        }
        catch (Exception ex) { _isAuto = prevAuto; SetStatus("Could not switch source: " + ex.Message); }
        finally { _switchingBase = false; UpdateBaseToggles(); }
    }
