        ...
        AiActionWindow.RunAndShow(System.Windows.Window.GetWindow(this), "批次加入影片",
            async (report, ct) =>
            {
                for (var i = 0; i < entries.Count; i++)
                {
                    ...
                    try
                    {
                        var cues = await Task.Run(() =>
                        {
                            var raw = TranscriptFile.Read(e.Path);                    // 免費解析直吃 raw——**不得先 StripToPlainText**（會刪 VTT <v 名字> 語音標記）
                            var parsed = SubtitleParser.Parse(raw);
                            if (!parsed.Any(c => c.StartSec.HasValue)) { parsed = SubtitleParser.ParseTimedTranscript(raw); }
                            return SubtitleParser.NormalizeOrder(SubtitleParser.ExtractInlineSpeakers(parsed));
                        }, ct);
    ...
    private async Task UpdateCurrentVideoTitleAsync()
    {
        ...
        try
        {
            ...
            if (double.TryParse(durRaw, NumberStyles.Float, CultureInfo.InvariantCulture, out var sec) && sec > 0)
            {
                ...
                if (_currentVideoItemId is not null) { _videoStore.UpdateDuration(_currentVideoItemId, sec); RefreshVideoList(); } // #207：實測長度落地（覆蓋推估），長度排序即時歸位
            }
        }
        catch { /* 取長度失敗維持字幕推估值 */ }
    }
...
""";
    ...
    private void SetWatchDurationFromCues()
    {
        ...
        if (last > 0 && _currentVideoItemId is not null) { _videoStore.UpdateDuration(_currentVideoItemId, last, estimateOnly: true); } // #207：推估先落（僅無值時），供長度排序
    }
