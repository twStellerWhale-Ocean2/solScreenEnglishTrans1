...
| orgSop | 通道 | 說明 |
| --- | --- | --- |
| orgSop#1-畫面選區查詢 | **桌面原生 GUI**（遮罩 overlay＋主視窗 Dictionary 分頁） | 熱鍵喚起框選、結果於 Dictionary 分頁查看聆聽（擷取完成主視窗前景疊於無邊框遊戲，#135） |
...
* **spec#4-可查看並聆聽查詢結果**：查詢結果於主視窗 **Dictionary 分頁**顯示（原文／音標／中譯），提供播放按鈕以 Windows 內建語音朗讀英文原文與中文翻譯、並可選擇朗讀語音（離線可用、免額度）；英文原文並可**逐字點選查詢該單字字義**（複查回饋改制：原為單獨發音；整句朗讀鈕並存，點字後可經往前鈕返回原句），並可**編輯原文重新翻譯**（辨識有誤時校正）；**並可於分頁頂部手動輸入英文查詢**（單字查字義、整句翻譯）；擷取完成時主視窗**自動前景**疊於無邊框視窗化遊戲之上（#135）。
...
* **e2eTest#02-畫面選區查詢一圈**（依 orgSop#1、docProgTest#02）：於無邊框視窗化前景應用按 `Alt+L`、框選英文區塊、等待查詢、於 Dictionary 分頁查看三欄結果、播放整句朗讀並點選單字聆聽發音（結果留於分頁、主視窗前景疊於遊戲）→ 全圈閉合、選區對位正確、結果三欄齊備、整句與單字發音皆可用、`ESC` 於擷取遮罩階段可逃（#135）。
...
* [etyCfg自訂sysLingoIsland組態]（應用情境）：命名情境存 `%APPDATA%\LingoIsland\contexts.json`（清單〔id／名稱／描述／圖片檔名／使用中〕），圖片存 `%APPDATA%\LingoIsland\contexts\`；非機密、可刪、不含金鑰（spec#9）。
...
| 系統匣選單頁（輔助） | 工具維保／teamSop#2.2·2.3 | 維運 | tray 圖示右鍵選單（**主視窗之輔助鏡像**）：狀態列（金鑰備妥／缺失）、開啟主視窗、Result（喚回結果卡，#107 兩入口鏡像）、查詢歷史、我的筆記、選項、關於、結束（開主視窗各項開至該分頁） | #2.2.1 | 系統匣 |
