...
* **影片擷取頁**：與螢幕擷取頁並列之擷取來源頁——**與截圖頁統一版面（#165／#167）左清單右操作**：左＝**影片清單到頂**（Clear all 置頂＋依 theme 篩選；載過影片可點切換、標記所屬主題、欄寬可拖拉，epic #145 增量4；**單項刪除改右鍵選單「Delete」或 `Delete` 鍵**〔`ListDeleteSupport`〕、無底部刪除鈕）｜右上＝**獲取區塊卡片**＝網址貼片列（Load）｜右下＝**內容區塊卡片**＝WebView2 播放器＋當前句字幕帶＋播放控制與**逐句英文字幕清單**。（螢幕截圖頁同模板：左＝截圖清單〔Clear all／theme 篩選／右鍵·Delete 刪〕｜右上＝獲取區塊〔Capture Screen＋快捷鍵 Change〕｜右下＝內容區塊〔放大預覽〕。**主題管理頁**〔#169〕亦同版面族：左＝主題清單〔New Theme 置頂／右鍵·Delete 刪〕｜右＝編輯區內容卡片。）逐句字幕清單**雙擊**該句才跳播（單擊僅選取，#169）；整檔 YAML 編修框自動換行（#169）。**說話人（epic #145 增量5）**：字幕若含說話人（人工字幕之 VTT `<v 名字>` 語音標記帶入、非推斷）則每句前置「說話人：內容」（清單與字幕帶皆然）；右欄上方**依說話人下拉篩選**（僅影響顯示、不動到句暫停 index）；可切**整檔 YAML 編修**（`speaker`／`start`／`text`〔start-only #158〕一次修訂斷句與說話人，Apply 解析回逐句、語法錯留編修態明訊）。沿用淺粉底＋半透明控制項與小女孩浮水印，字幕帶字級比照結果卡、暫停句逐字可點；載入／取字幕以進度與明確錯誤降級（循 [techApp桌面查詢工具] §B 不干擾介面 bar）。**多來源說話人疊加（epic #145 增量6）**：可插拔 `ISpeakerEnricher`（AI／wiki 等來源共用），影片頁「AI speakers」按鈕以 `OpenAiSpeakerEnricher` 依台詞逐句推斷、非破壞併回（僅補未標示、保留既有 ground truth；會用到 API 故按鈕觸發、標示為推斷非 ground truth）。wiki／網路台詞來源（同架構第二顆按鈕）為後續增量。**start-only（#158）**：對外字幕只留開始時間，一句顯示至下一句開始（無空窗）、到句暫停於「下一句開始，或本句開始＋上限（預設 8s，防超長間隔乾等）」（`PauseDecider`，顯示與暫停解耦；json3 併句/VTT 去滾動之結束時間僅解析階段內部 `TimedCue` 保留、對外丟棄）。**指定說話人才暫停（增量7）**：播放控制列「Pause at」下拉（Everyone＋各具名說話人），選定後只在該說話人之句到句暫停、其餘句略過續播。**依 theme 篩選（多媒體·B）**：影片清單與截圖清單頂端各一 theme 圖文下拉（縮圖＋名稱，共用 `ThemeFilter`），依選中 theme 篩選顯示（不動播放）。
...
* **docProgTest#07-影片擷取學習查詢**（orgSop#2）：開影片擷取分頁、貼入 YouTube 影片、取得字幕、導引播放到句暫停顯字幕、點字查詢、加入我的筆記；無字幕影片明確告知並中止。
...
| | teamSop#2.1 | prsnSop#2.1.1〔prsn玩家·影片擷取頁〕貼入 YouTube 影片、自動取得字幕、預處理為逐句〔[runWi自訂Usr選片取字幕]〕 |
...
| 影片擷取頁（本增量 spec#2） | 影片擷取／teamSop#2.1·2.2·2.3 | capture（影片）＋query＋present | 主視窗『影片擷取』分頁（與螢幕擷取〔情境〕頁並列之擷取來源頁）：**頂部**貼入 YouTube 連結／ID＋「載入」（載入即 `yt-dlp` 取字幕、無字幕明確提示並中止）；**左側**逐句英文字幕清單（可捲、點句跳播）；**右側** WebView2 嵌入之 YouTube 播放器＋**其下當前句字幕帶**（導引播放到句自動暫停、顯該句；續播／重播該句／下一句鈕）；暫停句字幕**逐字可點**——點詞即以既有查詢查該字（結果沿用 Dictionary 呈現與朗讀）、並可「加入我的筆記」（沿用去重/資料夾/發音練習、toast 回饋）。淺粉底＋半透明控制項一致；載入中／取字幕中顯進度、失敗顯明確錯誤與下一步 | #2.1.1·#2.2.1·#2.3.1 | 主視窗分頁（WebView2＋字幕清單） |
