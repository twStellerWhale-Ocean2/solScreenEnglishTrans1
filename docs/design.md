...
| 歷史分頁 | 遊戲查詢／teamSop#3.1·3.2·4.1 | present | 主視窗『歷史』分頁：左側依**日期分組**、右側該日條目（新在上、最舊在下，**緊湊列高**——單行原文＋時刻併列），每筆前端「＋筆記」（去重＋toast）、尾端 `▶ 播音`／`檢視`（回三欄中英）／`刪除`；頂部「清除全部」；空清單提示 | #3.1.1·#3.2.1·#4.1.1 | 主視窗分頁 |
