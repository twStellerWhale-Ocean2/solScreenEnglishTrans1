...
* **本增量（電子書來源）**：本方案再擴一擷取來源——**本機電子書（EPUB）**，與螢幕、影片並列之可插拔擷取來源。本增量交付**電子書增量1「獲得＋書櫃」地基**（`spec#4`–`spec#6`）：匯入本機 `.epub`（含批次）、解析容器/中繼（書名/作者/語言/dc:identifier）/spine/目錄（EPUB3 nav＋EPUB2 ncx）/封面、書櫃列出整理（封面縮圖、主題篩選、四鍵排序、依 dc:identifier 去重、刪除清空）、每本書一資料夾持久化並跨啟動還原、納入既有整包備份；**不含**章節閱讀渲染、逐段導讀、TTS 朗讀、逐字查詞、加入筆記、依說話人變色（皆列後續增量、另立工單；本增量【內容】子頁僅骨架）。
...
遊戲畫面（查詢對象、非介接系統）、OpenAI 平台（辨識翻譯外部服務）、Windows 內建語音（朗讀引擎）、**YouTube 影片（本增量之影片擷取來源；僅取字幕文字、不下載影片內容）**、**本機電子書檔（本增量之電子書擷取來源；使用者自選 `.epub`、僅讀取解析、原檔複本落地個人藏書資料夾、不改寫使用者原檔）**。
...
> 以表格描述本層動作／SOP（orgsopcat/orgSop→teamSop→prsnSop 逐層 zoom）；編號全程上下對應。orgSop 數＝需求域數（4：螢幕／影片／筆記／電子書）＋平台維保（1）＝5，與 spec（6：螢幕／影片／筆記各 1＋電子書增量1 三條 spec#4–#6）不等（維保為平台 orgSop、無獨立 value-spec；電子書增量1 一域 orgSop#5 對應三條增量 spec）。
...
| | orgSop#5-電子書擷取學習查詢 | 選定本機 EPUB 電子書為擷取來源、匯入解析（容器／中繼〔書名/作者/語言/dc:identifier〕／spine／目錄〔EPUB3 nav＋EPUB2 ncx〕／封面，含批次）、於書櫃瀏覽整理（封面縮圖／主題篩選／四鍵排序／依 dc:identifier 去重／刪除清空）、藏書每本一資料夾持久化跨啟動還原並納入整包備份；螢幕、影片與電子書為並列之可插拔擷取來源。**本增量交付「獲得＋書櫃」**（對映 spec#4–#6）；章節閱讀渲染、逐段導讀、TTS 朗讀、逐字查詞、加入筆記、依說話人變色為後續增量另立工單 |
...
* **本增量新增 techItem（電子書來源驅動，非 techApp 點名）**：[techItem電子書解析]（`VersOne.Epub`，解析 EPUB 2/3 之容器／中繼〔書名/作者/語言/dc:identifier〕／spine／目錄〔EPUB3 nav＋EPUB2 ncx〕／封面）——純受管、無外部行程；唯一新平台依賴＝`VersOne.Epub` NuGet 套件，具體選型於 ＜III.C.(A)＞ 落地。
...
| orgSop#5-電子書擷取學習查詢 | **桌面原生 GUI**（統一主視窗「電子書」分頁：【獲得】書櫃＋匯入卡片／【內容】閱讀骨架） | 選/拖 `.epub` 匯入（含批次）、解析中繼/spine/目錄/封面、書櫃列出整理（封面縮圖/主題篩選/四鍵排序/依 dc:identifier 去重/右鍵刪除清空）、藏書持久化與備份；章節閱讀渲染為後續增量 |
...
> need（spec#N，客戶目的／營運議題／成效期待，不混工程手段；粒度一致互不包含）＋端對端驗收課目（e2eTest，以 orgSop 為驗收單元、依 productReadme）。螢幕／影片／筆記複習各一 spec（spec#1–#3）、電子書增量1 三條 spec（spec#4–#6＝匯入／書櫃／持久化備份，同域 orgSop#5 承接）；平台維保為 orgSop#4、無獨立 value-spec（其行為由各 spec 之維運面承接、e2eTest#04 驗）。
...
* **spec#4-電子書匯入本機來源**：能把自己的**本機 EPUB 電子書**（EPUB 2 或 EPUB 3）當作與螢幕、影片並列之可插拔擷取來源——於「電子書」分頁**選檔或拖入 `.epub`**（可一次多檔批次），程式**讀取解析**其書名、作者、語言、章節目錄與封面（EPUB2 之 ncx 與 EPUB3 之 nav 目錄皆可、封面缺漏時明確以佔位呈現不當機），壞檔／非 EPUB／解析失敗明確告知並略過該檔、不中斷整批、不當機；本工具僅讀取解析、原檔複本落地個人藏書、不改寫使用者原檔。
* **spec#5-電子書書櫃瀏覽整理**：匯入之藏書能在**書櫃**一覽整理——每本以**封面縮圖＋書名／作者／章數**呈現，可**依主題篩選**、**依加入時間／書名／作者／主題四種排序**（同鍵可正反向、選擇跨啟動沿用）、**同一本書（依 dc:identifier）不重複入櫃**，可標記所屬主題、**右鍵或 `Delete` 刪除單本或清空書櫃**；書櫃與影片清單、截圖清單同一套「左清單右操作」版面族、易於在多本藏書間切換管理。
...
* **e2eTest#05-電子書獲得與書櫃一圈**（依 orgSop#5、docProgTest#08）：於「電子書」分頁【獲得】選檔或拖入一本 EPUB 3 與一本 EPUB 2 → 解析出書名/作者/章數/封面、彙總確認後匯入、於書櫃見封面縮圖卡；批次拖入多檔（含一本重複 dc:identifier 與一個非 EPUB 檔）→ 重複者略過、非 EPUB 明確標示不中斷整批；於書櫃切主題篩選、切四種排序、右鍵刪除一本、清空書櫃 → 顯示與持久化正確；重啟程式 → 書櫃原樣還原；「匯出資料…」→ zip 含藏書、於另一環境「匯入資料…」→ 書櫃還原 → 全圈閉合、壞檔明確降級不當機、原檔未被改寫。
...
* **spec#4（電子書匯入本機來源）**：`.epub` 匯入成功率、EPUB 2（ncx 目錄）與 EPUB 3（nav 目錄）皆解析、書名/作者/語言/dc:identifier/章數/封面欄位齊備率；封面缺漏以佔位呈現不當機、壞檔／非 EPUB／解析失敗明確告知並略過該檔且不中斷整批、不當機；批次匯入逐檔進度與結果摘要正確；僅讀取解析、原檔未被改寫稽核。
* **spec#5（電子書書櫃瀏覽整理）**：書櫃封面縮圖／書名／作者／章數呈現正確；主題篩選、四鍵排序（加入時間／書名／作者／主題正反向）即時且選擇跨啟動沿用；依 dc:identifier 去重（同書不重複入櫃）；右鍵／`Delete` 刪除單本與清空書櫃即時、與影片/截圖清單同版面族操作一致。
* **spec#6（電子書藏書跨啟動保存與備份）**：每本一資料夾落地（中繼／原始 `.epub`／封面）完整、重啟後書櫃清單與主題歸屬/排序偏好原樣還原；`ebooks.json` 或藏書資料夾毀損／不可寫退空/降級不致命、不影響其他分頁；藏書納入整包備份 zip（`BackupService` 辨識並涵蓋）、匯入還原後書櫃一致；金鑰不入藏書。
...
| team | 上級／編成位置 | 職責 |
| --- | --- | --- |
| team擷取學習（#1／#2／#3／#5） | Org使用者（遊戲中/回顧/整理時） | 熱鍵喚起、框選查詢、查看聆聽結果；回顧查詢歷史；收藏並整理我的筆記；建立管理應用情境並擇一使用；就地錄音練習發音；匯入解析本機 EPUB 電子書並於書櫃整理 |
...
| --- | --- |
| team擷取學習（#1／#2／#3／#5） | teamSop#1.1-熱鍵喚起與框選擷取<br>teamSop#1.2-辨識翻譯查詢<br>teamSop#1.3-結果查看與朗讀（逐字查/編輯重譯/打字查）<br>teamSop#1.4-情境建立與編輯（含圖片自動解釋）<br>teamSop#1.5-情境擇一使用<br>teamSop#2.1-影片選定與字幕預處理<br>teamSop#2.2-導引播放到句暫停查詢<br>teamSop#2.3-影片擷取加入筆記<br>teamSop#3.1-收藏加入我的筆記<br>teamSop#3.2-筆記整理（資料夾/排序/檢視/刪除）<br>teamSop#3.3-開啟並瀏覽查詢歷史<br>teamSop#3.4-查詢歷史清理<br>teamSop#3.5-筆記發音練習（錄音→AI 評分→成績框回饋→清空練習紀錄）<br>teamSop#5.1-電子書匯入與解析（選/拖 .epub、批次、中繼/spine/目錄/封面）<br>teamSop#5.2-書櫃瀏覽與整理（封面縮圖/主題篩選/四鍵排序/去重/刪除清空）<br>teamSop#5.3-藏書持久化與備份（每本一資料夾、跨啟動還原、納入整包備份） |
...
* **電子書解析元件（本增量）**：`VersOne.Epub` NuGet 套件（[techItem電子書解析]，import 進 WinApp、純受管、無外部行程；netstandard2.0 零相依、.NET 9 相容）——解析本機 EPUB 2/3 之容器/中繼/spine/目錄/封面；唯一新平台依賴僅此套件、無外部服務或行程。
...
* **techItem（本增量新增，電子書來源驅動）**：[techItem電子書解析]（`VersOne.Epub`，解析 EPUB 2/3 之容器/中繼/spine/目錄/封面）——import 進 WinApp、純受管、無外部行程、不單獨成容器；隨 Draft PR 請 USR 裁決入庫；具體版本／用法見 ＜III.C.(A)＞。
...
| 代號 | 測試對象 | 通過判定 |
| --- | --- | --- |
| cfgTest#01 | [etyCfg自訂sysLingoIsland組態] | 實作與部署組態符合契約規範（金鑰僅環境變數、appsettings 預設值正確、`paramHistoryMax` 非正值套用預設、`paramContextHint` 預設空且可往返、`paramPronPassThreshold` 預設 80 且可往返、`paramPronModel` 預設音訊模型且可往返、`ebooks.json` 書櫃清單與排序態預設且可往返、藏書資料夾路徑於 `%APPDATA%\LingoIsland\ebook\` 之下） |
...
* **docProgTest#08-電子書匯入與書櫃**（orgSop#5）：開電子書分頁【獲得】、選檔或拖入本機 `.epub`（單本與批次）、彙總確認後匯入、於書櫃見封面縮圖卡（書名/作者/章數）、切主題篩選與四種排序、右鍵刪除單本與清空書櫃、重啟後書櫃還原、經「匯出/匯入資料」整包備份帶走藏書；重複 dc:identifier 略過、非 EPUB/壞檔明確告知並略過該檔不中斷整批。
...
| | teamSop#5.1 | prsnSop#5.1.1〔prsn玩家·電子書擷取頁〕於【獲得】選檔或拖入本機 `.epub`（單本/批次）、解析容器/中繼/spine/目錄/封面、彙總確認後匯入落地藏書〔[runWi自訂Usr匯入電子書]〕 |
| | teamSop#5.2 | prsnSop#5.2.1〔prsn玩家·電子書擷取頁〕於書櫃瀏覽整理（封面縮圖/主題篩選/四鍵排序/依 dc:identifier 去重/右鍵刪除清空）〔[runWi自訂Usr瀏覽整理書櫃]〕 |
| | teamSop#5.3 | prsnSop#5.3.1〔prsn玩家·電子書擷取頁〕確認藏書每本一資料夾持久化、跨啟動還原並納入整包備份〔[runWi自訂Sys藏書持久化備份]〕 |
...
* **電子書解析（本增量）**：`VersOne.Epub` NuGet（[techItem電子書解析]；`EpubReader.OpenBookAsync` 惰性載入書櫃清單、`ReadBookAsync` 供後續章節渲染增量、`CoverImage`／`ReadingOrder`／`Navigation`／`Schema.Package.Metadata.Identifiers`／`EpubReaderOptions` 容錯；netstandard2.0 零相依、.NET 9 相容、純受管、無外部行程/服務）。
...
* [modEbook模組]（本增量）：沿用單一 WPF exe 之建置單元 [techStackDotnetWin]（modEbook 為 csproj 內一資料夾+namespace、不另立建置單元）；**電子書解析**＝[techItem電子書解析]＝`VersOne.Epub`（**NuGet id `VersOne.Epub`、版本 `3.3.6`、授權 Unlicense、目標框架 netstandard2.0 零相依、.NET 9 相容，附官方 WPF demo**）——用法要點：書櫃清單用 `EpubReader.OpenBookAsync` 回 `EpubBookRef` **惰性載入**（只取 `Title`／`AuthorList`／`CoverImage`、用完 `Dispose`）；後續章節渲染增量走 `ReadBookAsync`；封面 `EpubBook.CoverImage: byte[]?`；spine `ReadingOrder`；目錄 `Navigation`（nav/ncx 統一遞迴樹）；`dc:identifier` 走 `Schema.Package.Metadata.Identifiers`；壞檔容錯用 `EpubReaderOptions`。import 進 WinApp、不單獨成容器、無外部行程；契約為本增量新引、隨 Draft PR 請 USR 裁決入庫。另 [modQuery模組] 新增 `EbookStore`（`ebooks.json` 書櫃清單＋每本書資料夾 `ebook\{yyyyMMdd 標題}\` 落地〔`info.json`＋原始 `.epub` 複本＋封面圖〕，比照 `VideoStore`／`ScreenshotStore` 每項一資料夾範式；加入依 dc:identifier 去重、`SortEbooks` 四鍵排序投影、`Remove`／`Clear`、主題歸屬皆純函式抽可測、讀寫失敗退空/降級；納入 `BackupService` 整包備份）；[modPresent模組] 新增 `EbookPage`（比照 `VideoCapturePage` 之【獲得】/【內容】雙子頁：本增量【獲得】＝左書櫃清單〔`EbookListView`〕｜右匯入卡片、【內容】僅閱讀骨架）。
...
* **themes.json（#217 擴充）**：主題項增 `BlockedWords`（自動屏蔽字串；逗號分隔原字串、半形 `,`／全形 `，` 皆可；舊檔無此鍵＝空＝不過濾）——主題管理頁「主題描述」欄後新欄編輯；匯入字幕時以使用中主題之此欄過濾（見 ＜(C)＞ 影片擷取頁）。
...
| 電子書擷取頁（本增量 spec#4–#6） | 電子書擷取／teamSop#5.1·5.2·5.3 | capture（電子書）＋present（書櫃）；閱讀渲染為後續增量 | 主視窗『電子書』分頁（與螢幕擷取〔情境〕頁、影片擷取頁並列之第三個擷取來源頁）：**分【獲得】／【內容】兩子頁籤，疊放同格、以可見性互斥切換**（比照影片頁不用 `TabControl`）。**【獲得】子頁採「左書櫃清單｜右匯入卡片」版面**（同截圖頁「左清單右操作」統一版面族，本增量焦點）：**左＝書櫃清單**（`EbookListView`；到頂「清空書櫃」＋**主題篩選圖文下拉**〔共用 `ThemeFilter`〕＋**四鍵排序 toggle 鈕列**〔`加入時間`（新→舊，預設）／`書名`（A→Z 自然排序，不分大小寫）／`作者`（A→Z）／`主題`（主題名群組、未歸屬排末）；同鈕再點翻方向、作用中鈕以粗體＋▲/▼ 方向字圖標示〔非顏色線索、沿筆記 #126 家規〕、每模式各自記方向、存 `ebooks.json` 之 `Sort` 跨啟動沿用〕；每本書卡**封面縮圖＋書名／作者／章數／主題標籤**〔封面以 `OpenBookAsync` 惰性載入、缺封面顯佔位圖示〕、依 **dc:identifier 去重**、可點切換、可標記所屬主題、欄寬可拖拉；**單本刪除走右鍵選單「Delete」或 `Delete` 鍵**〔沿 `ListDeleteSupport`〕、無底部刪除鈕）｜**右＝匯入卡片**（【獲得】卡片：「選擇 EPUB 檔…」〔`OpenFileDialog`、`Multiselect`〕或**拖放 `.epub`**〔`AllowDrop`＋`DataFormats.FileDrop`、可多檔〕；**已選檔清單**〔標題列「已選 N 檔」＋「全部清除」，每列＝檔名＋解析出之書名/作者/章數＋**狀態欄**〔就緒／已在書櫃（同 dc:identifier）／非 EPUB 或解析不出容器／檔案不可讀〕＋逐項移除 `✕`〕；主行動鈕**文案隨可匯入檔數變化**〔單檔＝「匯入並開啟書櫃」／N 檔＝「匯入 N 本」〕，按下先出**批次彙總確認表**〔逐本可選 匯入／略過、已在書櫃者預設略過〕→匯入即解析中繼/spine/目錄/封面、每本落地一資料夾、加入書櫃；壞檔以 `EpubReaderOptions` 容錯、明確降級不中斷整批、原檔唯讀不改寫）。**【內容】子頁**＝閱讀渲染骨架（本增量僅保留段落原文落地；章節閱讀/逐段導讀/TTS 朗讀/逐字查詞/加入筆記/依說話人變色皆後續增量）。淺粉底＋半透明控制項＋小女孩浮水印一致；匯入中/解析中顯進度、失敗顯明確錯誤與下一步 | #5.1.1·#5.2.1·#5.3.1 | 主視窗分頁（書櫃＋匯入卡片） |
| 選項分頁 | 維運／teamSop#4.2 | 維運 | 主視窗『選項』分頁（即設定、版面美化），區塊序依使用頻率與關聯：①**快捷鍵**〔顯示當前喚起鍵綁定＋「變更」監聽鍵鼠、`Esc` 取消（開遮罩選取；第二熱鍵已於 Issue #90 移除）〕②**AI 辨識翻譯（OpenAI）**〔API 金鑰（寫使用者環境變數、不落地）＋查詢模型，同屬 AI 辨識〕③**朗讀語音**〔Windows 已安裝、含「測試發音」鈕〕④**發音練習**〔及格門檻（0–100、預設 80，滑桿或數字輸入）＋發音評分音訊模型 `paramPronModel`，本增量 spec#3〕⑤**資料備份與搬遷**〔#206：匯出資料…（整包 zip）＋匯入資料…（確認→覆蓋→關閉程式重啟套用）；即時動作、不走頁尾儲存；金鑰不在備份內之提示併陳〕；頁尾僅「儲存」；分組卡片、淺粉底（智能配色規則於 #69 移至情境頁各情境內、不在此）；**儲存成功以底部狀態列「Saved ✓」輕量閃示、不再彈模態框（#125）**；**未存變更離開守衛三選——存後離開／不存還原／取消（#125）**；**①快捷鍵『變更』監聽於 Windows 11 亦正確進入監聽並擷取鍵鼠（#127）**；**①快捷鍵區已移至螢幕擷取(Capture)頁、選項頁移除（#133，單一落點；擷取設定集中於擷取頁）** | #4.2.1 | 主視窗分頁 |
| 關於分頁 | 維運／teamSop#4.2 | 維運 | 主視窗『關於』分頁：上方資訊卡（程式名／版本、版權宣告、聯絡 email `carlton0521@gmail.com`、**GitHub 連結**〔點擊以預設瀏覽器開啟專案 repo，Issue #84〕、小女孩 logo、**更新區**——「檢查更新」鈕〔回「已是最新版本」／「新版 vX 已就緒」；失敗**如實回報**「無法檢查更新」〕、就緒時「立即重啟更新」鈕、dev 裸跑隱藏更新區）；**「更新紀錄（Change Log）」按鈕跳出獨立更新紀錄視窗**（`ChangeLogWindow`，modal／CenterOwner、可捲動唯讀；嵌入 `CHANGELOG.md` 為資源、執行期讀顯，Issue #79；**#159 由常駐佔頁區改按鈕跳出**，About 頁只留資訊卡；**#199 此鈕置於更新區「檢查更新」之後**〔仍於更新區 `StackPanel` 之外恆顯、不隨更新區於 dev 裸跑一併隱藏〕；**#216 與更新區併為同一列**〔外層橫排容器包更新區＋此鈕；更新區 Collapsed 不佔位、鈕自然靠列首〕） | #4.2.1 | 主視窗分頁 |
...
| 55 | runWi自訂Usr管理影片清單（排序） | 49 | 【內容】左欄主題篩選下之**排序下拉**：預設`加入時間`＝清單新在前（現況行為、升級無感）；切`名稱`→依標題自然排序（不分大小寫、數字段依數值）；切`時間長度`→短→長、**無長度值排末**（舊資料載入該片起播後 `DurationSec` 自動補值、下次排序即歸位）；切`主題`→依主題名群組（未歸屬排末、組內新在前）；選擇落地 `videos.json` 之 `SortKey`、重啟沿用；排序與主題篩選並存（先篩後排）、目前載入片仍自動選中；排序為呈現層投影、`Upsert` 插入序不變（`SortVideos` 純函式測涵蓋：四鍵、null 長度混排、穩定排序、`SortKey` 舊檔相容）|
| 56 | runWi自訂Usr導引播放點字查詢（快速鍵與導航暫停） | 50 | 【內容】頁按 `Space` → 播放⇄暫停切換（同 `li_toggle`）；`←`／`→` → 跳上一句／下一句並自該句起播；焦點在獲得頁輸入框／時間偏移框／YAML 編修框打字時 `Space`／方向鍵**不被劫走**、影片清單 `Delete` 刪除保留；**指定說話人暫停模式下按 `←` 跳至非勾選句 → 播完該句於句末暫停、不彈回原勾選句**（導航一次性 override；停達後恢復勾選規則——續播自動依勾選停下個目標句）；導航至未定時句 → 顯示該句、影片暫停原地不閃跳、按繼續自原處續播（override 判定沿用 `PauseDecider.NextPause` 全部句規則，純函式測涵蓋「導航句非勾選句仍於句末停」情境；快速鍵接線與 WebView2 焦點限制以實機手測驗）|
| 57 | setWi自訂Usr安裝設定金鑰（資料備份與搬遷） | 01 | 選項頁「匯出資料…」→ 產生單一 zip（含 `notes.json`／`themes\` 等整包內容、**不含任何金鑰**）、預設檔名帶日期；目的地刻意選 `%APPDATA%\LingoIsland` 自身 → zip 不含自己（先壓暫存再搬置）；「匯入資料…」選該 zip → 確認對話（載明覆蓋與關閉重啟）→ 同名檔覆蓋、非同名檔保留、完成後程式關閉，重啟後筆記／主題／影片清單與設定同匯出機；選**任意非備份 zip** → 明確拒絕「不是 LingoIsland 備份檔」、本機資料一檔不動；（roundtrip／識別／不吃自己／拒絕路徑皆 `BackupService` 純函式＋暫存目錄測涵蓋；對話與關閉重啟流程實機手測）|
| 58 | runWi自訂Usr匯入電子書（單本解析） | 02 | 電子書分頁【獲得】以「選擇 EPUB 檔…」給入一本 **EPUB 3**（nav 目錄）→ 解析出書名／作者／語言／dc:identifier／章數／封面、摘要確認後匯入、書櫃見封面縮圖卡；另試一本 **EPUB 2**（ncx 目錄）→ 亦解析章目；封面缺漏之 EPUB → 以佔位圖示呈現、不當機；非 EPUB／容器解不出／檔案不可讀 → 明確標示並略過、不當機；使用者原檔未被改寫（`VersOne.Epub` 解析：容器/中繼/spine `ReadingOrder`/目錄 `Navigation` nav+ncx/`CoverImage`/`Schema.Package.Metadata.Identifiers`/`EpubReaderOptions` 容錯，以小型測試 `.epub`／假 `EpubBookRef` 注入，去重 key/章數/中繼萃取為純函式測涵蓋）|
| 59 | runWi自訂Usr匯入電子書（批次） | 58 | 【獲得】一次選/拖多個 `.epub`（含刻意混入之**重複 dc:identifier** 一本與**非 EPUB** 一個）→ 已選檔清單逐列顯檔名/書名/作者/狀態、可逐項 `✕` 與「全部清除」；按「匯入 N 本」→ 彙總確認表逐本可選 匯入／略過（重複者預設略過）→ 確認後批次匯入書櫃；重複 dc:identifier 不重複入櫃、非 EPUB 標示不中斷整批、逐檔進度與結果摘要（成功/略過/失敗）正確；原檔一律唯讀不改寫（彙總結果組建、去重、狀態判定為純函式可測；拖放手勢以檔案選取路徑覆蓋等價邏輯、拖放另於 Q3 端對端截圖佐證）|
| 60 | runWi自訂Usr瀏覽整理書櫃 | 58 | 書櫃切**主題篩選**（共用 `ThemeFilter`）只看該主題藏書；切**四鍵排序** `加入時間`／`書名`（自然排序、不分大小寫）／`作者`／`主題`（未歸屬排末），同鍵再點翻方向、作用中鈕以粗體＋▲/▼ 標示（非顏色線索），選擇落地 `ebooks.json` 之 `Sort`、重啟沿用；標記某本所屬主題；右鍵「Delete」或 `Delete` 鍵刪單本→自書櫃與其資料夾移除；「清空書櫃」→ 清單清空；排序與主題篩選並存（先篩後排）、目前選中片仍自動選中（`SortEbooks` 四鍵/正反向/未歸屬排末/穩定排序純函式測涵蓋）|
| 61 | runWi自訂Sys藏書持久化備份 | 58 | 匯入後每本落地一資料夾 `ebook\{yyyyMMdd 標題}\`（`info.json`＋原始 `.epub` 複本＋封面圖）；重啟程式 → 書櫃清單、主題歸屬、排序偏好原樣還原；`ebooks.json` 或藏書資料夾毀損／不可寫 → 書櫃退空/降級、其他分頁不受影響、不當機；選項頁「匯出資料…」→ zip **含 `ebooks.json` 與 `ebook\` 藏書**（`BackupService.CreateBackup` 辨識涵蓋），於另一環境「匯入資料…」→ 書櫃還原一致（roundtrip／識別／退空降級皆 `BackupService`／`EbookStore` 純函式＋暫存目錄測涵蓋；關閉重啟流程實機手測）|
