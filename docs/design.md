...
* [YouTube影片]（[modVideoCapture模組] 對外）：yt-dlp 取字幕／WebView2 導引播放（本增量）。
...
| 影片擷取頁（本增量 spec#2） | 影片擷取／teamSop#2.1·2.2·2.3 | capture（影片）＋query＋present | 主視窗『影片擷取』分頁（與螢幕擷取〔情境〕頁並列之擷取來源頁）：**頂部**由逐字稿（貼字幕檔網址）：輸入字幕檔網站／網址（單檔或目錄頁多連結）→OpenAI `web_search`〔gpt-4.1〕解析驗證含說話人→`yt-dlp` 依標題定位、結果表點列 Load 載入；字幕檔原始 URL 帶入候選（增量2）；取字幕仍 `yt-dlp`、無字幕明確提示並中止；**左側**逐句英文字幕清單（可捲、點句跳播）；**右側** WebView2 嵌入之 YouTube 播放器＋**其下當前句字幕帶**（導引播放到句自動暫停、顯該句；續播／重播該句／下一句鈕）；暫停句字幕**逐字可點**——點詞即以既有查詢查該字（結果沿用 Dictionary 呈現與朗讀）、並可「加入我的筆記」（沿用去重/資料夾/發音練習、toast 回饋）。淺粉底＋半透明控制項一致；載入中／取字幕中顯進度、失敗顯明確錯誤與下一步 | #2.1.1·#2.2.1·#2.3.1 | 主視窗分頁（WebView2＋字幕清單） |
