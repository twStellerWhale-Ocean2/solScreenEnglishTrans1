...
| 44 | runWi自訂Usr管理我的筆記（底色盤擴為十色） | 43 | [NoteColors.Palette] 擴為**十色**（原五色 hex 不變、零遷移；新五色 Violet `#E9DCF6`/Orange `#FDE7D4`/Mint `#D9F1F0`/Lime `#EFF6D5`/Tan `#F0E5D8`）→ 四消費點**隨盤自動沿用**：筆記右鍵選單列 `No Color`＋十色（12 項＋分隔線＋`Delete` 居末）、點新色（如 `Violet`）→ 卡片套色並落地 `notes.json`；結果視窗底部色塊列 11 塊**全可視可點**（過寬換行、不裁切）；情境配色規則區塊十色各一格；AI 建議色名清單含新色（`NormalizeSuggested` 認得新色名/hex、非盤上值仍回空）；舊 `notes.json`（僅舊五色）開啟不變 |
