...
* **統一主視窗（常駐主控入口）**：程式執行後保留一個**工作列按鈕型**的可見主視窗——常顯於工作列、可 Alt+Tab／點工作列尋得；採 **Office 式頂部功能列分頁（每鈕圖示在上、文字在下：📒筆記／🕘歷史／⚙選項／ℹ關於）＋下方對應功能頁**，承載維運與檢視；**預設最小化、不擋遊戲**，需要時還原。此入口不依賴 Win11 系統匣顯示設定，換版／換路徑後仍穩定可尋（系統匣圖示改為輔助入口）。**關閉主視窗＝收合（最小化/隱藏至系統匣）而非結束程式**，唯明確「結束」才退出常駐。
...
| 查詢結果頁 | 遊戲查詢／teamSop#1.2·1.3·4.1 | query＋present | 淺粉底圓角大字卡片（可縮放、記住位置大小；預設約 560×380）：查詢中＝`辨識翻譯中…`；完成＝三區直排（原文／KK 音標／中譯），英文原文逐字可點（點詞即單獨發音、游標呈可點狀），英文組與中文組各附獨立整句播放鈕與「自動播放」勾選；**卡片底部工具列**置「加入我的筆記」按鈕與「自動加入筆記」勾選（類比自動播放，勾選後查詢成功即去重加入並 toast）；**不再置歷史／我的筆記入口**（改由統一主視窗）；失敗＝錯誤訊息＋下一步指引 | #1.2.1·#1.3.1·#4.1.1 | 桌面浮動視窗（topmost、可拖曳縮放） |
| 統一主視窗（Office 式殼） | 工具維保／teamSop#2.2 | 維運 | **標準工作列視窗**（`ShowInTaskbar`、可最小化收合、關閉（✕）＝收合非結束、點工作列／Alt+Tab 還原）：**頂部功能列分頁**（每個分頁鈕採**圖示在上、文字在下**〔📒筆記／🕘歷史／💬情境／⚙選項／ℹ關於；實作以 Segoe MDL2 Assets 字形圖示或 emoji〕）＋下方對應功能頁；淺粉底（承結果卡片色）＋小女孩 logo 背景浮水印；含金鑰狀態列。取代原各獨立維運/檢視視窗，為第一線常顯主控入口 | #2.2.1 | 桌面視窗（工作列按鈕、分頁殼） |
| 筆記分頁 | 遊戲查詢／teamSop#4.2 | present | 主視窗『筆記』分頁：左側**多層資料夾樹**（標準 `TreeView`＋`HierarchicalDataTemplate`，可新增子夾／更名／刪除、**拖曳移動節點如檔案總管**、防自我或子孫巢狀）、右側選取夾之條目（新在上、可拖曳排序），每筆 `▶ 播音`／`檢視`（回三欄中英）／`刪除`；收藏由結果頁「加入我的筆記」寫入、toast 回饋 | #4.1.1·#4.2.1 | 主視窗分頁 |
| 歷史分頁 | 遊戲查詢／teamSop#3.1·3.2·4.1 | present | 主視窗『歷史』分頁：左側依**日期分組**、右側該日條目（新在上、最舊在下），每筆前端「＋筆記」（去重＋toast）、尾端 `▶ 播音`／`檢視`（回三欄中英）／`刪除`；頂部「清除全部」；空清單提示 | #3.1.1·#3.2.1·#4.1.1 | 主視窗分頁 |
...
| 選項分頁 | 工具維保／teamSop#2.2 | 維運 | 主視窗『選項』分頁（即設定、版面美化）：API 金鑰（寫使用者環境變數、不落地）、朗讀語音〔Windows 已安裝〕、查詢模型、**喚起快捷鍵**〔顯示當前綁定＋「變更」監聽鍵鼠、`Esc` 取消〕；分組卡片、淺粉底 | #2.2.1 | 主視窗分頁 |
...
| 16 | runWi自訂Usr管理我的筆記（資料夾/排序） | 15 | 開我的筆記 → 新增／更名／刪除資料夾、把條目歸入某夾；拖曳條目上下 → 順序改變並存 `notes.json`、重啟後沿用；空資料夾／跨夾移動邊界正確 |
