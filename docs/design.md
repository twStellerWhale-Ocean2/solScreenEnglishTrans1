...
| 45 | runWi自訂Usr管理我的筆記（條目選取回饋/加大字） | 43 | 筆記/歷史兩頁條目**單擊**→卡片框轉**深粉 `#B0578D`**（框厚恆定 2px 不跳版、任意底色可辨）；點另一卡→選取移轉（單選）；**既有互動迴歸不受影響**——雙擊仍開檢視（首擊即選取）、右鍵選單照常、行尾播音/麥克風鈕不觸發選取（不冒泡）、筆記拖曳排序照常；字級：筆記原文 16/時間小字 11.5、歷史原文 15.5/時刻 12（截圖視覺驗）；選取不持久化（切夾/重繪即清）、不掛刪除等新行為 |
